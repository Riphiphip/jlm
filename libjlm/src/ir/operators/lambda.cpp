...
node::ComputeCallSummary() const
{
  ...
  jive::result * rvsdgExport = nullptr;
  ...
  while (!worklist.empty()) {
    ...
    auto inputNode = jive::input::GetNode(*input);
    if (is<CallOperation>(inputNode) && input == inputNode->input(0)) {
      directCalls.emplace_back(AssertedCast<CallNode>(inputNode));
      continue;
    }

    auto result = dynamic_cast<jive::result*>(input);
    if (result != nullptr
    ...
    auto simpleInput = dynamic_cast<jive::simple_input*>(input);
