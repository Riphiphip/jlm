...
{
  using DirectCallsConstRange = iterator_range<std::vector<CallNode*>::const_iterator>;
  using OtherUsersConstRange = iterator_range<std::vector<jive::simple_input*>::const_iterator>;

public:
  CallSummary(
    jive::result * rvsdgExport,
    std::vector<CallNode*> directCalls,
    std::vector<jive::simple_input*> otherUsers)
...
private:
  jive::result * RvsdgExport_;
  std::vector<CallNode*> DirectCalls_;
  std::vector<jive::simple_input*> OtherUsers_;
};
